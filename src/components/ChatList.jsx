  ...
  return (
    <div className="flex-grow overflow-y-auto">
      <ul className="space-y-1 p-2">
        {chats.length === 0 ? (
          <li className="text-gray-500 text-sm p-3">No chats yet. Create one to get started.</li>
        ) : (
          chats.map(chat => (
            <li key={chat.id}>
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-md flex items-center space-x-3 transition-colors 
                  ${activeChatId === chat.id ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-gray-700 hover:text-gray-100 text-gray-300'}`}
              >
